...
from requests.exceptions import ReadTimeout
from config.BG_process_status import songDownloader, skip, Objects
from config.config import Authorization
from config.state import next_coming
...
def check_song_duration(search_query: str, expected_duration: int | None = None) -> Tuple[Any, Any] | None:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            for entry in info.get("entries", []):
                ...
                if score > best_score:
                    ...
                    best_entry_duration = duration

            if best_entry_url:
                print(f"✅ Selected: {best_entry_name} ({best_entry_url}) (Score {best_score:.1f})")
                return best_entry_url, best_entry_duration
...
async def song_downloader():
    from config.requestHandler import get_request, remove_request
...
async def download_song_from_id(track_id, requester, length):
    ...
    from config.songHandler import save_to_next_coming, next_coming_file, load_json, save_json
    ...
    try:
        ...
        video_url, video_duration = check_song_duration(f"{track_name_sanitized} {artist_name} {album_name}", length)
        
        next_coming.append(video_url)
...
def get_next_song_from_playlist():
    ...
    from config.songHandler import save_to_next_coming, track_already_played_id
    from config.blocker import is_song_blocked
...
async def download_song_from_playlist():
    ...
    from config.songHandler import next_coming_file, load_json, save_json
    ...
    video_url, video_duration = check_song_duration(search_query, track_duration_sec)
